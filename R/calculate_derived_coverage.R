...
calculate_derived_coverage <- function(.data, indicator) {
  ...
  data <- .data %>%
    # select(any_of(c(group_vars)))
    select(any_of(c(group_vars, 'year', population, 'population_yoy_change', 'population_survey_change',
                    'population_proportion', indicator, penta1_denom, penta1_derived_denom, cov_penta1, cov_penta1_derived)))
